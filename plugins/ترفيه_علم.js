...
╯───────────────────────╰ـ`.trim(), conn.tekateki[id][0]);
    ...
    try {
        ...
        const fileId = '1NTc2WRnutuqFq69ODxr2CVuxheVl62zz';
        ...
        if (res.data && Array.isArray(res.data)) {
            ...
            let json2 = tekateki[Math.floor(Math.random() * tekateki.length)];

            

            let _clue = json2.name;

            let clue = _clue.replace(/[A-Za-z]/g, '_');

            let img = json2.img;

            let answer = json2.name;

             let questions = json2.question || 'ما إسم هذه الشخصية ؟';
