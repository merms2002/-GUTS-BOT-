const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  const textpremERROR = `*اعـــــمــل مـــنــشـــن لــلـــشــــخــص...⛩️*\n\n*—◉ مثل:*\n*◉ ${usedPrefix + command} @${m.sender.split`@`[0]} 1*\n*◉ ${usedPrefix + command} 1 <منشن>*`;
  ...
  if (command == 'بريم' || command == 'userpremium') {
    ...
    const textprem1 = `*تـــــهــــانـيــــنا...لــــقـــد أصـبــــحــت مـــمــيــز🥳*\n\n*✨ الاسم: ${name}*\n*🕐 الوقت: ${txt} ساعات*\n*📉 الثواني: ${timeLeft} ثواني*`;
  ...
  if (command == 'بريم2' || command == 'userpremium2') {
    ...
    const textprem2 = `*تـــــهــــانـيــــنا...لــــقـــد أصـبــــحــت مـــمــيــز🥳*\n\n*✨ الاسم: ${name}*\n*🕐 الوقت: ${txt} ساعات*\n*📉 الثواني: ${timeLeft} ثواني*`;
  ...
  if (command == 'بريم3' || command == 'userpremium3') {
    ...
    formatTime(user.premiumTime - now).then((timeleft) => {
      const textprem3 = `*تـــــهــــانـيــــنا...لــــقـــد أصـبــــحــت مـــمــيــز🥳*\n\n*✨ الاسم: ${name}*\n*🕐 الوقت: ${txt} ساعات*\n*📉 الثواني: ${timeLeft} ثواني*`;
  ...
  if (command == 'بريم4' || command == 'userpremium4') {
    ...
    user.premium = true;
    formatTime(user.premiumTime - now).then((timeleft) => {
      const textprem4 = `*تـــــهــــانـيــــنا...لــــقـــد أصـبــــحــت مـــمــيــز🥳*\n\n*✨ الاسم: ${name}*\n*🕐 الوقت: ${txt} ساعات*\n*📉 الثواني: ${timeLeft} ثواني*`;
